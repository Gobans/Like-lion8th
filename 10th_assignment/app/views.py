...
def result(request):
    ...
    elif query and setting == "contents":
        BlogPosts = BlogPosts.filter(Q(text__icontains = query)).order_by('-time')
    elif query and setting == "author":
        BlogPosts = BlogPosts.filter(Q(author__username__icontains = query)).order_by('-time')
    return render(request, 'result.html',{'BlogPosts' : BlogPosts, 'query':query,'setting':setting,'user':user,})